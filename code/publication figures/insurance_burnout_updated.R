...
dat <- dat %>% mutate(health_insurance_cat = relevel(health_insurance_cat, ref = "1,000-2,000"))
...
plot_outcome_1 <- combi_1 %>% group_by(model) %>%
  # filter(clean_name == "ISCO Occupation label") %>% # Can restrict figure to only some of the variables
...
# Print the plot ####
emf(file = here("output", "publication figures", "Health insurance.emf"),width = 15.5, height = 8.5, bg = "white")  # full is width = 14.7, height = 8
