...
dat <- dat %>% mutate(
  health_insurance_cat = relevel(health_insurance_cat, ref = "300"), 
  exercise_regularly = factor(case_when(
                        exercise_hard_dich == "Regularly" | exercise_moderate_dich == "Regularly" ~ "Regularly",
                        .default = "Not regularly"),
                        levels = c("Regularly", "Not regularly"))
                      )
...
uv_mbi <- uv_eembi_log$table_body %>% select(variable, var_type,  reference_row, row_type, N, 
  ...
  mutate(estimate = round(estimate,2),
         ...
         new_variable = case_match(       # relabel to look nice for the plots
           variable,
           "health_insurance_cat" ~ "Insurance premium in CHF",
           ...
           "exercise_regularly" ~ "Exercise frequency"
         ),
...
#              clip = c(0.33, 3),                               # Cut off the x-axis
#              vertices = TRUE,
#              ci.vertices = TRUE,
...
#              xticks = c(log(0.33), log(0.5), log(0.666), log(1), log(1.5), log(2), log(3)),
#              xticks.digits = 1,
#              grid = structure(c(1), 
...
emf(file = "insurance_combined without CI.emf",width = 9.8, height = 7, bg = "white")  # full is width = 15, height = 8
...
emf(file = "insurance_combined with CI.emf",width = 14.7, height = 8, bg = "white")  # full is width = 14.7, height = 8
## Combine the two figures
grid.newpage()
