...
## Overall values ####
table_overall <- dat %>% select(all_of(var_included)) %>% select(-wfh_exposure) %>% 
...
#   modify_header(all_stat_cols() ~ paste0("**{level}**","\nN = {n}"))
# ## Add clean names
# clean_table <- sex_table$table_body %>% left_join(clean_labels) %>% 
#   mutate(label = if_else(row_type == "label", clean_name, label)) %>% select(-clean_name) %>% 
#   mutate(delete = case_when(
#     variable == "burn_out" & label == "No" ~ 1,
#     variable == "burnout_interp_dich30" & label == "Not Severe" ~ 1,
...
#     .default = NA
#   )) %>% filter(is.na(delete))
# sex_table$table_body <- clean_table ; rm(clean_table)
# sex_table %>% as_flex_table()
...
table_combined2 <- table_combined %>% 
  ...
  hline(i = c(5, 42, 59), border = fp_border_default(color = "black")) %>% # lines to indicate variable themes
  # bg(part = "all", bg = "grey") %>%
  ...
  width(j = c(2,3,6,7), width = 1.9, unit = "cm") %>% 
  width(j = c(4:5), width = 1.6, unit = "cm") %>%
  line_spacing(space = 0.1, part = "body") %>%
  valign(valign = "bottom", part = "body") ; table_combined2

# Save as docx ####
save_as_docx(
  "Table 1: Participant summary" = table_combined2, path = here("output", "publication figures", "Exposure table_main_TW.docx"))



## Combine for main table - TW group + sex ####
# !! Try to get it so that only the levels have a different color, and the labels are white !!
table_combined <-
  tbl_merge(
    tbls = list(table_overall, 
...
table_combined2 <- table_combined %>% 
  ...
  # autofit() %>%
  # set_table_properties(width = .1, layout = "autofit") %>%
  vline(j = c(2,5), border = fp_border_default(color = "grey")) %>%
  # bg(part = "all", bg = "grey") %>%
  bg(i = groupnames, part = "body", bg = "#EFEFEF") %>% 
  # bg(part = "header", bg = "lightgrey") %>% 
  bg(i = c(14,22) ,part = "body", bg = "white") %>%      # no background for continuous variables (count out the rows...)
  ...
  width(j = c(2:4,10), width = 1.8, unit = "cm") %>% 
  width(j = c(5:9), width = 1.6, unit = "cm") %>% 
  line_spacing(space = 0.1, part = "body") %>%
  valign(valign = "bottom", part = "body") ; table_combined2

# Save as docx ####
save_as_docx(
  "Table 1: Participant summary" = table_combined2, path = here("output", "publication figures", "Exposure table_main_large.docx"))
