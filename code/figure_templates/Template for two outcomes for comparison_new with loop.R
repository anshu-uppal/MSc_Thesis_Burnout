...
# adjusted odds ratios in a forest plot for two outcomes using the same exposure and covariates
# In the final plots, outcome 1 is on the left, outcome 2 is on the right
...
# # Only take participants from the SP1,2,3,4 and pop-pilote
# dat <- dat %>% filter(serocov_representative == TRUE)
...
# dat <- dat %>% filter(!percent_change_cat %in% c("Reduced"))
...
outcomes <- c("burnout_interp_dich30", "burn_out")
...
### ### ### ### ### ### ### ### ### ### ### ### ### ###
# Run a for loop for each outcome ####
### ### ### ### ### ### ### ### ### ### ### ### ### ###
...
mv_model_1 <- c(exposure, covariates) %>%     ## begin with vector of exposure and covariate column names
  ...
  str_c(outcomes[i]," ~ ", .) %>%        ## combine the names of variables of interest with outcome_1 in formula style
...
plot_data[i] <- list(combi_1)
}




# Forest plot Outcome 1 ####

plot_outcome_1 <- plot_data[[1]] %>% group_by(model) %>%
...
# Forest plot Outcome 2 ####
plot_outcome_2 <- plot_data[[2]] %>% group_by(model) %>%
  filter(clean_name %in% c("Teleworking group")) %>% # Can restrict figure to only some of the variables
  filter(!clean_name %in% c("ISCO Occupation label")) %>% # Can restrict figure to only some of the variables
...
# Print the plot ####
emf(file = here("output", "publication figures", "Full OR figure_MBI30_TWG_ref Never.emf"),width = 14.4, height = 2.5, bg = "white")  # full is width = 15.5, height = 8.5
## Combine the two figures
grid.newpage()
pushViewport(viewport(layout = grid.layout(nrow = 1, ncol = 2, widths = c(1.43,1)))) # set 1.6 for full
