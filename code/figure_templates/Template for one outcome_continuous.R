...
dat <- dat %>% mutate(hh_livewith_rec_en = relevel(hh_livewith_rec_en, ref = "Couple with children"),
                      wfh_updated = factor(wfh_updated, levels = c("Occasionally (irregular)", "1+ days/week", "Every day", "Never",  "Not possible")))
...
mv_model <- c(exposure, covariates) %>%     ## begin with vector of exposure and covariate column names
  ...
  glm(family = "gaussian",                  ## define the family as binomial for logistic regression
      data = dat) %>%                         ## define your dataset
  tbl_regression(exponentiate = FALSE) ; mv_model
...
uv_estimates <- uv_model$table_body %>% # Extract the estimates table
  ...
  select(clean_name, var_type,  reference_row, row_type, N, 
         label, n_obs, estimate, conf.low, 
         conf.high, ci, p.value) %>% 
  filter(row_type == "level") %>% 
  mutate(ci = paste0(format(round(conf.low, 2), nsmall = 2),", ", format(round(conf.high,2), nsmall = 2)),
         model = "Unadjusted",
         ...
         n_obs_uv = n_obs,
         n_obs_mv = NA,  # Same for mv, but it's empty here and will be filled in later steps
...
combined_plot <- combi %>% group_by(model) %>%
  ...
  forestplot(labeltext = c(new_label, 
                           n_obs_uv, full_estimate_uv, 
                           n_obs_mv, full_estimate_mv
                           ),
             fn.ci_norm = c(fpDrawNormalCI, fpDrawCircleCI),  # Set shapes of the symbols
             lty.ci = c(1, 2),                                  # Set the linetypes
             clip = c(-3.5, 5),                               # Cut off the x-axis
             vertices = TRUE,
             ci.vertices = TRUE,
             ci.vertices.height = 0.07,
             boxsize = .3,
             graphwidth = unit(2, "inches"),   # Set graphwidth
             colgap = unit(3, "mm"),
             line.margin = .2, # Add this to avoid crowding
             xlog = FALSE,      # Show x-axis on log scale
  ...
  fp_add_header(new_label = c("", ""),
                n_obs_uv = c("", "N") %>% fp_align_center(),
                full_estimate_uv = c("Unadjusted", "beta (95% CI)") %>% fp_align_center(),
                n_obs_mv = c("", "N") %>% fp_align_center(),
                full_estimate_mv = c("Adjusted", "beta (95% CI)") %>% fp_align_center()
  ) %>%
  fp_set_zebra_style("#EFEFEF", ignore_subheaders = TRUE) ; combined_plot


# # Print image to PDF
# pdf(file = "Template_continuous outcome.pdf",
#     width = 9.5, height = 7,  # Play around with dimensions
#     paper = "a4r",   # Can set the orientation to landscape
#     colormodel = "cmyk") ; combined_plot ; dev.off()

emf(file = here("output", "publication figures", "Continuous MBI_no contract changes.emf"),
    width = 10, height = 2.5, bg = "white") ; combined_plot ; dev.off()


# # Plot of covariates ------------------------------------------------------
# covariate_plot <- combi %>% 
...
#                            n_obs_uv, full_estimate_uv
#   ),
#   lty.ci = c(1),                                  # Set the linetypes
#   clip = c(-3, 7.5),                               # Cut off the x-axis
#   vertices = TRUE,
#   ci.vertices = TRUE,
...
#                 full_estimate_uv = c("Unadjusted", "beta (95% CI)") %>% fp_align_center()
#   ) %>%
#   fp_set_zebra_style("#EFEFEF", ignore_subheaders = TRUE) ; covariate_plot
# 
# emf(file = here("output", "publication figures", "Continuous MBI_covariates.emf"),
#     width = , height = 8.5, bg = "white") ; covariate_plot ; dev.off()
