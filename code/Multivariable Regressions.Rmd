---
title: "Multivariable Regressions"
author: "Anshu Uppal"
...
* `wfh_updated` re-categorized into three groups (Never, Not Possible, Yes) (`wfh_trich`)  
  * The `wfh_updated` variable was made from the `wfh` variable by cleaning the "Never" responses  
    * Cleaning process was to assign the most relevant ISCO-08 code I could find for each of the free-text professions.  
...
* `hh_livewith_rec_en` --> reported living situation but not taking into account age of children
* `hh_livewith_rec_en_children` --> reported living situation but also taking into account age of children (young defined as $\le$ 10 years)
* `num_young_children_dich` --> do they have young children (aged 10 and younger)?
