...
table_percent_change_cat <- dat %>% 
  ...
  bold_labels() %>% 
  modify_spanning_header(all_stat_cols() ~ "**Change in working hours**") %>%
  modify_footnote(all_stat_cols() ~ NA) %>% 
  # modify_header(all_stat_cols() ~ paste0("**{level}**","\nN = {n}\n({style_percent(p)}%)")) %>%
  modify_header(all_stat_cols() ~ paste0("**{level}**","\nN = {n}"))
## Add clean names
clean_table <- table_percent_change_cat$table_body %>% left_join(clean_labels) %>% 
  mutate(label = if_else(row_type == "label", clean_name, label)) %>% select(-clean_name) %>% 
  mutate(delete = case_when(
    variable == "burn_out" & label == "No" ~ 1,
    variable == "burnout_interp_dich30" & label == "Not Severe" ~ 1,
...
table_percent_change_cat$table_body <- clean_table ; rm(clean_table)
table_percent_change_cat %>% as_flex_table()

## Combine for main table ####
# !! Try to get it so that only the levels have a different color, and the labels are white !!
table_combined <-
  tbl_merge(
    tbls = list(table_overall, table_percent_change_cat),
    tab_spanner = c(NA, "Change in working hours")
  ) 
...
table_combined2 <- table_combined %>% 
  ...
  width(j = c(2:6), width = 1.8, unit = "cm") %>% 
  line_spacing(space = 0.1, part = "body") %>%
  valign(valign = "bottom", part = "body") ; table_combined2

# Save as docx ####
save_as_docx(
  "Table 1: Participant summary" = table_combined2, path = here("output", "publication figures", "sensitivity figures",  "decreased hours.docx"))
