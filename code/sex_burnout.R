...
# dat %>% group_by(age_cat) %>% count()
dat <- dat %>% filter(!age_cat %in% c("18-25", "66+")) %>%
  mutate(age_cat = factor(droplevels(age_cat), levels = c("56-65", "46-55", "36-45", "26-35")
  ))
...
b <- a %>% filter(sex_en == "Male")
a <- a %>% filter(sex_en == "Female")
...
mv_eembi_log_f <- tbl_regression(m2, exponentiate = TRUE)
...
uv_mbi_f <- uv_eembi_log_f$table_body %>% select(variable, var_type,  reference_row, row_type, N, 
...
mv_mbi_f <- mv_eembi_log_f$table_body %>% select(variable, var_type,  reference_row, row_type, N, 
                                             label, n_obs, n_event, estimate, conf.low, 
                                             conf.high, ci, p.value) %>% 
  filter(row_type == "level")%>% 
  mutate(estimate = round(estimate,2),
...
uv_mbi_f <- uv_mbi_f %>% mutate(full_estimate_mv = mv_mbi_f$full_estimate_mv
                            , n_comb_mv = mv_mbi_f$n_comb_mv
                            , N_mv = mv_mbi_f$N_mv
                            , n_perc_mv = mv_mbi_f$n_perc_mv
)
mv_mbi_f <- mv_mbi_f %>% mutate(full_estimate_uv = uv_mbi_f$full_estimate_uv
                            , n_comb_uv = uv_mbi_f$n_comb_uv
                            , N_uv = uv_mbi_f$N_uv
                            , n_perc_uv = uv_mbi_f$n_perc_uv
)

# Row bind the two tables 
combi_f <- rbind(uv_mbi_f, mv_mbi_f)
combi_f <- combi_f %>% mutate(full_estimate_uv = str_remove_all(full_estimate_uv, "[*]"),
                          full_estimate_mv = str_remove_all(full_estimate_mv, "[*]"))


## Final forest plot with both values in one figure ####
ee_mbi_plot_f <- combi %>% group_by(model) %>%
  ...
  fp_set_zebra_style("#EFEFEF", ignore_subheaders = TRUE) ; ee_mbi_plot_f
