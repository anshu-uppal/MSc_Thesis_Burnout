...
dat <- tibble(readRDS(here("data", "clean_dataset.rds"))) %>% 
  mutate(Occupation_label_2 = relevel(factor(Occupation_label_2), ref = "Business and administration professionals"),
...
combined_plot <- combi %>% group_by(model) %>%
  filter(clean_name != "ISCO Occupation label") %>% 
